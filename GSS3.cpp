...
void update ( int vertex , int lseg , int rseg , int index , int oldval){
  if ( index > rseg || index < lseg )
    return ;
  if ( lseg == rseg && lseg == index ) {
    seg[vertex].fullsum = seg[vertex].maxsum = seg[vertex].lsum = seg[vertex].rsum = in[lseg] ;
    return ;
  }
  update ( vertex<<1 , lseg , (lseg+rseg)>>1 , index , oldval ) ;
  update ( (vertex<<1)+1 , ((lseg+rseg)>>1)+1 , rseg ,index , oldval ) ;
  seg[vertex].fullsum = seg[vertex<<1].fullsum + seg[(vertex<<1)+1].fullsum ;
  seg[vertex].maxsum = max ( max ( seg[vertex<<1].maxsum , seg[(vertex<<1)+1].maxsum ), seg[vertex<<1].rsum + seg[(vertex<<1)+1].lsum ) ;
  seg[vertex].lsum = max ( seg[vertex<<1].lsum , seg[vertex<<1].fullsum + seg[(vertex<<1)+1].lsum ) ;
  seg[vertex].rsum = max ( seg[(vertex<<1)+1].rsum , seg[(vertex<<1)+1].fullsum + seg[vertex<<1].rsum ) ;
}
